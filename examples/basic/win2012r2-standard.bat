...
SET ISO_URL=iso\en_windows_server_2012_r2_with_update_x64_dvd_4065220.iso
SET ISO_CHECKSUM=
...
IF NOT EXIST "%ISO_URL%" (
    @ECHO.
	@ECHO Windows Server 2012 R2 ISO file not found:
    @ECHO.
	@ECHO    %ISO_URL%
	GOTO :eof
)
...
packer.exe build -only=hyperv-iso -var "iso_url=%ISO_URL%" -var "iso_checksum=%ISO_CHECKSUM%" win2012r2-standard.json
